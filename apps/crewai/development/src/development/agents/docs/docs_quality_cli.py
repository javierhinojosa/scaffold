...
def main():
    ...
    parser.add_argument('--github-token', help='GitHub token for Giscus integration (optional)', 
    ...
    file_parser = subparsers.add_parser('file', help='Review a single documentation file')
    file_parser.add_argument('file_path', help='Path to the documentation file')
    file_parser.add_argument('--repo-name',
                            help='GitHub repository name (e.g., "owner/repo"), required if posting to GitHub')
    file_parser.add_argument('--discussion-category',
                            help='GitHub Discussions category ID, required if posting to GitHub')
    
    # Directory review command
    dir_parser = subparsers.add_parser('directory', help='Review all documentation files in a directory')
    dir_parser.add_argument('dir_path', help='Path to the documentation directory')
    dir_parser.add_argument('--repo-name',
                           help='GitHub repository name (e.g., "owner/repo"), required if posting to GitHub')
    dir_parser.add_argument('--discussion-category',
                           help='GitHub Discussions category ID, required if posting to GitHub')
    ...
    if args.post_to_github:
        if not args.github_token:
            print("Error: GitHub token is required when posting to GitHub. Set it via --github-token or GITHUB_TOKEN environment variable")
            return 1
        if not args.repo_name or not args.discussion_category:
            print("Error: --repo-name and --discussion-category are required when posting to GitHub")
            return 1
    ...
    session = agentops.init(
        skip_auto_end_session=True  # Only use valid parameters
    )
    
    # Create the docs review agent
    agent = DocsReviewAgent(github_token=args.github_token if args.post_to_github else "dummy_token")
    ...
    try:
        if args.command == 'file':
            ...
            if args.post_to_github and result:
                posted = agent.post_to_giscus(args.repo_name, args.discussion_category, result)
