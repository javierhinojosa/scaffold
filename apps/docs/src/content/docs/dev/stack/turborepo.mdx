...
2. **Development**
   ...
   - Runs in persistent mode with caching disabled
...
1. **Running Tasks**
   ...
   pnpm build --filter @sfh/package-name
...
2. **Formatting**
   ...
   pnpm format:js    # JavaScript/TypeScript files
   pnpm format:py    # Python files
   pnpm format:misc  # Other file types
   ```
...
- `/tsconfig.json` - Root TypeScript configuration (referenced in global dependencies)
