...
export const { signInWithEmail, signUp, signOut, getCurrentUser, onAuthStateChange } = auth;
```
...
test('should handle invalid credentials', async () => {
  await expect(auth.signInWithEmail('invalid@email.com', 'wrongpass')).rejects.toThrow();
});
```
...
test('should update auth state', async () => {
  const {
    data: { subscription },
  } = auth.client.auth.onAuthStateChange(callback);
  try {
    // Test state changes
  } finally {
    subscription.unsubscribe();
